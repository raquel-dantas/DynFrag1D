...
# BC dictionary
bc_dict = {2: (0, "dirichlet"), 4: (-vel, "velocity"), 5: (vel, "velocity")}
...
alpha_critical = (
    stress_critical**2
    + 4.5 * strain_rate ** (2 / 3) * E * Gc ** (2 / 3) * rho ** (1 / 3)
) / (4.5 * Gc)
...
sigmac = np.random.uniform(
    low=stress_critical - 1.0 * 10**6,
    high=stress_critical + 1.0 * 10**6,
    size=(n_el),
)
...
# Contact penalty
alpha = [
    (sigmac[i] ** 2 + 4.5 * strain_rate ** (2 / 3) * E * Gc ** (2 / 3) * rho ** (1 / 3))
    / (4.5 * Gc)
