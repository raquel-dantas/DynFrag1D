...
def Newmark_exp(M, u, v, acel, d, p_next, dt):
    ...
    epsilon = [(u[DFMesh.connect[el][1]] - u[DFMesh.connect[el][0]]
    ...
    # const =({'type': 'ineq', 'fun': lambda d: -(d[i] - d[i+1] - DFMesh.ElemLength(i)/lip_constraint)},
    #         {'type': 'ineq', 'fun': lambda d: -(d[i] - d[i-1] - DFMesh.ElemLength(i)/lip_constraint)}
    # )
        ...
        minimize(
            ...
            constraints=(
                {'type': 'ineq', 'fun': lambda d:
                    -(d[i] - d[i+1] - DFMesh.ElemLength(i)/lip_constraint)},
                {'type': 'ineq', 'fun': lambda d:
                    -(d[i] - d[i-1] - DFMesh.ElemLength(i)/lip_constraint)}
            )
        ).x
    ...
    x = [(DFMesh.node_coord[el+1] + DFMesh.node_coord[el])
         * 0.5 for el in range(DFMesh.n_el)]
    # y = x
