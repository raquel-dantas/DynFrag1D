...
def Run_simulation(strain_rate):
    ...
    for n in range(DFMesh.n_steps):
        ...
        Epot[n], Ekin[n], Edis[n], Wext[n] = DFPostprocess.Energy(
            up_bc_left,
            up_bc_right,
            u,
            v,
        ...
        for bc in range(len(DFMesh.materials)):
            if DFMesh.materials[bc] == 4 or DFMesh.materials[bc] == 5:
                if DFMesh.materials[bc] == 4:
                    elbc = 0
                    up_bc_left = np.array(
                        [u[DFMesh.connect[elbc][0]], u[DFMesh.connect[elbc][1]]]
                    )
                    dp_bc_left = d[elbc]
                else:
                    elbc = DFMesh.n_el - 1
                    up_bc_right = np.array(
                        [u[DFMesh.connect[elbc][0]], u[DFMesh.connect[elbc][1]]]
                    )
                    dp_bc_right = d[elbc]
    ...
    varEkin, varEpot, varEdis, varWext, varEtot = DFPostprocess.VarEnergy(
        Epot, Ekin, Edis, Wext
    )
